...
int main(int argc, char **argv)
{
    ...
    if (argc != 8)
    {
        ...
        nbTries = 10;
        h = 1.0;
        factor = 2.0; // 31 15 3 5 10 1.0 1.0
    }
    else
    {
        // Parse command line arguments
    ...
    auto projDepMerit = std::make_unique<ProjMeritFASTWAFOM>(w, q, std::move(table_c), 5);

    auto figure = std::make_unique<WeightedFigureOfMerit<ProjMeritFASTWAFOM>>(std::numeric_limits<Real>::infinity(), std::move(weights), std::move(projDepMerit));

    //! [random_search_column_by_column]
